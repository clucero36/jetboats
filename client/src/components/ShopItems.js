...
const ShopItems = ({ items }) => {
  ...
  // Displayed to User
  return (
    <Box p='3rem 0' backgroundColor='orange.100' borderRadius='xl' m='0 auto' w='90%'>
      <Heading align='center' m='.75rem 0' color='orange.800' fontFamily='Lucida Console'>JBC Gear</Heading>
      <Grid templateColumns={['repeat(2, 1fr)','repeat(4, 1fr)', 'repeat(4, 1fr)', 'repeat(4, 1fr)']} gap='10' w={['80%','80%','80%', '50%']} m='0 auto'>
        {renderedShopItems}
      </Grid>
    </Box>
  )
...
const ShopItem = ({ item }) => {
  ...
  return (
    <GridItem colSpan='2' borderRadius='full' boxShadow='dark-lg' m='0 auto' >
      <VStack backgroundColor='orange.100' >
        <Box align='left' w='100%'>
          <Image src={item.img} borderRadius='md' maxH='350px'/>
          <Box pl='.25rem'>
